...
void move_by_tag(char *source,char *destination)
{

int hastagfile; //need to see if .pm_tag file exists

hastagfile = FileExists(source);
if(hastagfile == 1)
{
...
if( argc == 4 ) {
      ...
      {
      if(strcmp(arg1,"move_by_tag")==0){ //0 means the string are the same so is a valid function
      move_by_tag(arg2,arg3);
      }
      else 
      {
